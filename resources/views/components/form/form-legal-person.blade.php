<div class="field-set @if($this->type != App\Enums\Person\PersonType::JURIDICA->value) d-none @endif">
    <div class="row">
        <div class="col-lg-3 col-12">
            ...
            <input wire:model.defer="cnpj" value="{{ $this->cnpj }}" class="form-control form-control-sm cnpj" required @if($this->disableInputs) disabled @endif>
        ...
        <div class="col-lg-3 col-12">
            ...
            <input wire:model.debounce.500ms="tradingName" value="{{ $this->tradingName }}" class="form-control form-control-sm" required @if($this->disableInputs) disabled @endif>
    ...
    <div class="row">
        <div class="col-lg-3 col-12">
            ...
            <select wire:model.debounce.500ms="stateRegistrationCategory" value="{{ $this->stateRegistrationCategory }}" class="form-control form-control-sm" required @if($this->disableInputs) disabled @endif>
        ...
        @if($this->type == App\Enums\Person\PersonType::JURIDICA->value)
            <div class="col-lg-3 col-12">
                ...
                <input wire:model.debounce.500ms="ie" value="{{ $this->ie }}" class="form-control form-control-sm" @if(App\Enums\Person\StateRegistrationCategory::tryFrom($this->stateRegistrationCategory)->required()) {{ 'required' }} @else {{ 'disabled '}} @endif @if($this->disableInputs) disabled @endif>
                @error('ie') <span class="error">{{ $message }}</span>@enderror
            </div>
        @endif
        <div class="col-lg-3 col-12">
            <label class="control-label">Inscrição Municipal</label>
            <input wire:model.debounce.500ms="im" value="{{ $this->im }}" class="form-control form-control-sm" required @if($this->disableInputs) disabled @endif>
            @error('im') <span class="error">{{ $message }}</span>@enderror
        </div>
        <div class="col-lg-3 col-12">
            <label class="control-label">Situação CNPJ</label>
            <input wire:model.debounce.500ms="cnpjStatus" value="{{ $this->cnpjStatus }}" class="form-control form-control-sm" id="cnpj" required @if($this->disableInputs) disabled @endif>
